...
from rest_framework.fields import SerializerMethodField, ImageField, CharField, \
...
class RecipeReadSerializer(ModelSerializer):
    ...
    ingredients = IngredientInRecipeReadSerializer(many=True, source='recipeingredient_set')
...
class RecipeWriteSerializer(ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if ingredients_data is None or len(ingredients_data) == 0:
            raise ValidationError({"ingredients": "This field cannot be empty or missing."})

        if tags is None or len(tags) == 0:
            raise ValidationError({"tags": "This field cannot be empty or missing."})
...
class UserWithRecipesSerializer(ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email', 'username', 'first_name', 'last_name', 'is_subscribed', 'recipes', 'recipes_count', 'avatar']
